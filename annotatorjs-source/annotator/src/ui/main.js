...
var customDrawing = require('./customdrawing')
...
function main(options) {
    ...
    function start(app) {
        ...
        s.customDrawing = new customDrawing.CustomDrawing({onSave: function(objects) {
          // var annotation = {type: "customDrawing", objects: objects}
          // app.annotations.create(annotation);
        }, onUpdate: function(annotation) {
          app.annotations.update(annotation);
        }, onCreate: function(object) {
          // var annotation = {type: "customDrawing", object: object}
          app.annotations.create(object);
        }});
    ...
    return {
        ...
        annotationsLoaded: function (annotations) {
          const result = annotations.filter(ann => ann.tpt_type !== "customDrawing");

          s.highlighter.drawAll(result);

          const customAnnotations = annotations.filter(ann => ann.tpt_type === "customDrawing");
          s.customDrawing.drawAll(customAnnotations)
        },
        annotationCreated: function (annotation) {
          if(annotation.tpt_type === "customDrawing") {
            s.customDrawing.updateAnnotation(annotation)

            console.log("Its a custom drawing.")
            return
          }
        ...
        annotationUpdated: function (annotation) {
          if(annotation.tpt_type === "customDrawing") {
            console.log("annotationUpdated: Its a custom drawing.")
            return
          }
           s.highlighter.redraw(annotation); 
          },
